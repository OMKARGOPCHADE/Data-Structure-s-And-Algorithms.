...
int removeDuplicates(vector<int> &nums)
{
    ...
    for (int i = 0; i < nums.size(); i++)
    {
        ...
        if (k > 2)
        {
            auto it1 = find(nums.begin(),nums.end(),nums[i]);
            it1+=2;
            //it1++;
            k-=2;
            nums.erase(it1,it1+k);
        }
    }
    for (auto i : nums)
        cout << i << " ";
